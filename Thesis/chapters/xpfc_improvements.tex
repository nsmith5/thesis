...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{ \nc{Adding a Free Energy of Mixing} } %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
%
\nc{where $\epsilon$ is a parameter that is possibly temperature dependent.} This form results in a free energy functional of the form,
%
\begin{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\nc{Generalizing the Two-Point Correlation Function} } %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation}
%
\nc{where the index $i$ runs over the components. For example, in the exact theory that emerges from the original alloy CDFT theory (equation \ref{binaryCs}),} we have,
%
\begin{gather}
...
\end{gather}
%
\nb{confusing notation: $\zeta_{IJ}(c)$ in Eqs. 5.6-5.8 and $\zeta_i(c)$ in Eq. 5.6; how do they relate?} 
Each interpolating function, $\zeta_i(c)$, defines a domain of validity for its
associated correlation function $C_i(r, r^\prime)$. \nc{This suggests the following new 
definition that we introduce herein to generalizethe density-density correlation function 
for a binary alloy: we introduce a set of correlation functions, indexed by $C_i(r, r^\prime)$,
each of which model a crystal structure that is known to manifest itself in an alloy system. 
For each, there is an associated interpolation function $\zeta_i(c)$ that defines the range of 
concentrations over which these correlations are valid. In principe, $\zeta_i(c)$ can also be temperature dependent, although we do not consider that case in this thesis. }
...
XPFC formalism for modelling bare correlation functions \nc{(i.e. equation \ref{XPFC_C2})}. The $\alpha$ and
...
driving force for eutectic solidification, our simplified regular \nc{solution XPFC} model allows
...
the pair correlation function used \nc{ in the binary XPFC of Greenwood \textit{et al.}} with a
...
diagram like that in Fig. \ref{eutectic}.  \nb{Say that the diagram shows the binodal and spinodal limes and define what they mean.}
...
\begin{figure}[h]
    ...
    \caption[Eutectic Phase Diagram]{
        ...
        $\beta$ solid phases. \nc{$l$ denotes the liquid}. The free energy parameters are $\eta = 2$, $\chi = 1$,
        $\omega=0.02$, $\epsilon_0 = 26.6$ and $T_c = 0.15$. The parameters of the
        structure functions are $\sigma_{10\alpha} = \sigma_{10\beta} = 0.8$,
        $k_{10\alpha} = 2\pi$, $k_{10\beta} = 4\pi/\sqrt{3}$ and $T_0 = 1$. \nb{blue should be "equilibrium coexistence" in the inset, and perhaps draw a horizontal line at the eutectic temp and say "Horizontal line denotes eutectic temperature".}
    }
\end{figure}
\nc{It is noted that the phase diagram in Fig. \ref{eutectic} and in what follows were done using the same approach that was used in numerous PFC literature \cite{GREENWOOD11_BINARY}. The approach is as follows: a mode expansion for the density is assumed for each crystal phases (zero amplitudes for the liquid phase). For each  phase, an amplitude equation analogous to equation \ref{amplitude_gp} results, except in this case, it is a function of amplitudes, average density {\it and} concentration. This coarse grained free energy of each phase is then minimized with respect to the amplitudes,  leaving a free energy density for each phase that is a function only of the average density and concentration. At this juncture, we simplify matters by assuming that the average density is a constant for the system. We then minimize the total free energy of the system with respect to concentration, assuming a conserved total concentration field. This latter step considers separately the coexistence of  (1) $\alpha$-liquid, (2)  
$\beta$-liquid, (3) $\alpha$-$\beta$ over different temperature/concentration ranges. Original code was developed to carry our these phase diagram constructions, and implemented using Mathematica.}  

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Syntectic Phase Diagram} %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Our regular solution XPFC model also allows for the study of a variety of invariant binary
...
can be done by choosing \nc{a single interpolation function to be} a window 
...
of parameters can be seen in Fig. \ref{syntectic}. \nb{Make a comment why it doesn't connect to the spinodal.}
...
solid and solute rich liquid. To model a monotectic using our \nc{regular solution XPFC 
model} we hypothesize a solid phase at $c=0$ and set the spinodal temperature higher
...
free energy is a crucial element in this complete description. \nc{it is also noteworthy 
that the approach introduced here is extendable in a straightforward way to multi-component
alloys if the interpolation functions become multivariate functions of the concentrations.}
