...
\nc{(for that reference density)} and $\bar{n}$ is the fractional density 
...
The amplitudes are constrained by the \nc{point group symmetries of the lattice}.
...
lattice vectors, \nc{and $\lbrace\mathbf{G}\rbrace_\alpha$ represents the set 
of reciprocal lattice vectors transforming under the symmetry element of the point group
labelled by $\alpha$.}

If we insert equation \ref{amplitudes} into equation \ref{gp} and integrate
over the unit cell \nc{of the particular crystal we wish to develop the theory for,} 
we find,
%
\begin{align}
    \label{amplitude_gp} 
...
\end{equation}
%
\nb{had forgotten the subscript $l$ and $C_0^2$?} 
...
\begin{table}[H]
    ...
    \caption[Table of Ramakrishnan results]{Freezing parameters for fcc and
        ...
        expansion in the free energy \nb{does this mean uses a three-point direct correlation function?}. $\eta$ is the fractional density change of
...
\end{equation}
%
Where $\mean{\cdot}_{ne}$ denotes the non-equilibrium average, \nc{(i.e., using $f(\q, \p,t)$)}. Just as the
...
\end{equation}
%
\nb{$f(\q,\p,t)$ should be $f(\q,\p,t')$ in Eq.(3.15), no?} in which $\hat{\mathbf{J}}(r, t)$ is the local density flux,
%
\begin{equation}
...
for numerical algorithms \nc{that aim to explore long-time microstructure evolution}. 
...
on a fine mesh. \nc{This further restricts the time scales of microstructure evolution 
that can be practicality explored to  times scales comparable to those of molecular 
dynamics --perhaps somewhat longer.}
...
the Phase Field Crystal (PFC) theory, \nc{the topic of this thesis, aims to 
achieve} precisely this balance.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Phase Field Crystal Theory} %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The phase field crystal theory (PFC) presents a solution to the aforementioned numerical
difficulties faced by DDFT methods by approximating the free energy in such a
way as to retain the basic features of the theory using a smoother solid state
description of density . Starting with the approximate free energy functional of equation
...
average \nb{also assumes that $\mu$ is expanded about $\mu_o$, and dropping higher order susceptibility terms, right?}.  Most phase field crystal theories also use a simplified equation of
motion as well,
%
\begin{equation}
    \label{pfc_eom}
...
\nc{As alluded to above, these two simplifications formally make the PFC theory different from CDFT, turning it instead into a type of Ginzgurb-Landau type of field theory, where $n$ represents an order parameter that becomes periodic in the solid state. As has been shown in the PFC literature, this apparently gross over-simplification of CDFT manages to correctly reproduce many of the qualitative physics of solidification, such as nucleation, grain boundary misorientation energy, elastic response and dislocations in the solid phase, vacancy diffusion and creep, grain boundary pre-melting, vacancy trapping, and numerous other effects. It is thus hoped that by progressively improving on the parameterization of  PFC theories, guided always by inspection of the underlying  forms and parametrization of CDFT and DDFT, it will be possible to eventually develop a PFC theory that also captures the physics of the aforementioned processes quantitatively.}