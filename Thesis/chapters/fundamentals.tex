...
%
\begin{gather}
    \Omega_{id}[\psi] = -\f{k_bT}{\Lambda^{3}} \int\mathrm{d}x\,e^{\beta\psi(x)} \\ 
    \F_{id}[\rho] = k_bT\int \mathrm{d}x\l\lbrace \rho(x) \ln\l(\Lambda^3\rho(x)\r) - \rho(x) \r\rbrace,
\end{gather}
% 
...
\begin{equation}
    \F_{ex}[\rho] = \F_{ex}[\rho_0]
        ...
        + \f{1}{2} \Delta\rho(x^\prime) \ast \l . \f{\d^2 \F_{ex}}{\d\rho(x)\d\rho(x^\prime)}
...
% QS: reference equations or elaborate here. Got lost
%
...
% QS: Needs something to conclude the chapter and put it all together.
