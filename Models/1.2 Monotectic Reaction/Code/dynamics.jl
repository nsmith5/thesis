...
function ΔFmix(c::Float64, s::State)
	ϵ = -4.0 + s.ϵ₀*(s.σ - s.σ₀)
	return s.ω*(c*log(c/s.c₀) + (1-c)*log((1-c)/(1-s.c₀)) + 0.5*ϵ*(c-c₀)^2)
end

δΔFmixδc(c::Float64, s::State) = s.ω*(log(c/s.c₀) - log((1-c)/(1-s.c₀)))
...
function set_nonlinear!(s::State)
	third = 1/3
	for i in 1:s.N*s.N
		s.nₙₗ[i] = s.n[i]^2*(-0.5*s.η + third*s.χ*s.n[i]) + ΔFmix(s.c[i], s) - exp(-(s.c[i])^2/(2s.αc^2))*s.C₂n[i]
		s.cₙₗ[i] = (s.n[i] + 1.)*δΔFmixδc(s.c[i], s) - 0.5*s.n[i]*(-(s.c[i])/s.αc^2*exp(-(s.c[i])^2/(2s.αc^2))*s.C₂n[i])
	end
end
